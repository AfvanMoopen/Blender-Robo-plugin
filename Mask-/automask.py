...
class test_helper:
    ...
    def test(self, context, model, state, movpath):
...
class OBJECT_OT_test_single(Operator):
    bl_idname = "object.test_single"
    bl_label = ""
    ...
    def execute(self, context):
        ...
        proj_dir = paths[-1]
        if proj_dir == '':
            raise ValueError('test path is empty.')
        state = proj_dir  # set first state to proj_dir
        model = None
        ret = amh.test(context, model, state, movie_details)
        if type(ret) == set:
            return ret
        del ret
        return {'FINISHED'}
    ...
    def modal(self, context, event):
        if event.type in {'RIGHTMOUSE', 'ESC'}:
            ...
            if bpy.context.scene.frame_current < frame_end:
                try:
                    ret = self.amh.test(context, self.model, self.state, self.amh.movpath)
                except ValueError:
...
class PANEL0_PT_test(Panel):
    bl_label = "Mask Tracking"
    bl_idname = "PANEL0_PT_test"
    ...
    def draw(self, context):
        ...
        split = split.split()
        c = split.row()
        c.operator("object.test", icon="TRACKING_FORWARDS")
        c.operator("object.test_single", icon="TRACKING_FORWARDS_SINGLE")
...
classes = (OBJECT_OT_test_single, OBJECT_OT_test, OBJECT_OT_clear_forwards, OBJECT_OT_clear_backwards, PANEL0_PT_test, Settings)
